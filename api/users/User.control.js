...
module.exports = {
    ...
    getUserById: (req, res) => {
        ...
        userById(req.id, (err, results) => {
            ...
            if (!results) {
                return res.status(404).json({ msg: "Record not found Mom" });
            }
            return res.status(200).json({ data: results })
        })
    },
    login: (req, res) => {
        ...
        if (!email || !password)
            return res
                .status(400)
                .json({ msg: 'Not all fields have been provided! Mom' })
        ...
        getUserByEmail(email, (err, results) => {
            ...
            //creating token for the signed user that expires in 1 hour and using our secret key for creation to make logout with in 1 hour
